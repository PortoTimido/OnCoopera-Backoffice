...
import { Eye, ListFilter, Pencil, Plus, ShieldBan, Trash2, UserCheck, UserX } from 'lucide-react'
import { Link, useSearchParams } from 'react-router-dom'
import { getApiErrorMessage } from '../../../shared/api/httpClient'
import { deleteBackofficeAdministrator, listBackofficeUsuarios, type PaginatedUsuarios } from '../../backoffice/api/backofficeApi'
...
export function UserListPage() {
  ...
  async function handleDelete(id: string, name: string) { if (!window.confirm(`Inativar o administrador "${name}"?`)) return; try { await deleteBackofficeAdministrator(id); setReload((value) => value + 1) } catch (deleteError) { setError(getApiErrorMessage(deleteError)) } }
  return (
