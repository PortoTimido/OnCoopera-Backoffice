...
export type SupportImage = { id: string; url: string; ordem: number }
export type SupportResource = { id: string; nome: string; tipoApoio: SupportCategory; telefone: string; descricao: string | null; status: SupportStatus; endereco: SupportAddress; horarios: SupportSchedule[]; imagensUrl: string[]; imagens: SupportImage[]; estaAbertoAgora: boolean; distanciaKm?: number; dataCriacao: string; dataAtualizacao: string }
export type SupportFormValues = { name: string; category: SupportCategory; phone: string; description: string; cep: string; street: string; number: string; neighborhood: string; city: string; state: string; coordinates: Coordinates; schedules: SupportSchedule[] }
export type PaginatedSupports = { data: SupportResource[]; page: number; pageSize: number; total: number; totalPages: number }
export type SaveSupportPayload = Pick<SupportResource, 'nome' | 'tipoApoio' | 'telefone' | 'descricao' | 'endereco' | 'horarios'> & { status?: SupportStatus }
