...
export function SupportScheduleCard({ schedule, onChange }: { schedule: SupportSchedule[]; onChange: (schedule: SupportSchedule[]) => void }) {
  ...
  function addSchedule() {
    if (!startTime || !endTime) { toast.error('Informe o horário de início e fim.'); return }
    if (startTime >= endTime) { toast.error('O horário de início deve ser anterior ao fim.'); return }
  ...
  return <section className="grid gap-4 rounded-3xl bg-white p-6 shadow-[4px_4px_0_rgba(187,202,196,0.45)]">
    ...
    <div className="grid gap-3 rounded-xl bg-surface-mint p-3"><AdminCreateSelect buttonClassName={fieldButtonClass} className="text-xs font-bold text-admin-text" label="Dia da semana" menuClassName="z-20" onChange={setDay} options={dayOptions} tone="adminField" value={day} /><div className="grid grid-cols-2 gap-2"><AdminCreateSelect buttonClassName={fieldButtonClass} className="text-xs font-bold text-admin-text" label="Início" menuClassName="z-20" onChange={setStartTime} options={timePickerOptions} tone="adminField" value={startTime} /><AdminCreateSelect buttonClassName={fieldButtonClass} className="text-xs font-bold text-admin-text" label="Fim" menuClassName="z-20" onChange={setEndTime} options={timePickerOptions} tone="adminField" value={endTime} /></div><button className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-brand-teal px-3 text-sm font-bold text-white" onClick={addSchedule} type="button"><Plus size={16} />Adicionar horário</button></div>
