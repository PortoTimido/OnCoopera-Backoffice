...
export function SupportEditorPage() {
  ...
  const imageSaveError = new URLSearchParams(location.search).has('imageUploadFailed') ? 'Apoio salvo, mas não foi possível persistir a foto. Selecione-a novamente para tentar de novo.' : ''
  ...
  useEffect(() => {
    ...
    const id = supportId
    let active = true
    async function loadSupport() { setIsLoading(true); setError(''); try { const result = await getBackofficeSupport(id); if (active) { setSupport(result); setValues(mapSupportToForm(result)); setIsPhotoRemovalRequested(false) } } catch (loadError) { if (active) setError(getApiErrorMessage(loadError)) } finally { if (active) setIsLoading(false) } }
    void loadSupport()
    return () => { active = false }
  }, [supportId])
