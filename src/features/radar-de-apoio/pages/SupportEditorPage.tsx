...
export function SupportEditorPage() {
  ...
  useEffect(() => {
    ...
    const id = supportId
    let active = true
    async function loadSupport() { setIsLoading(true); setError(''); try { const result = await getBackofficeSupport(id); if (active) { setSupport(result); setValues(mapSupportToForm(result)) } } catch (loadError) { if (active) setError(getApiErrorMessage(loadError)) } finally { if (active) setIsLoading(false) } }
    void loadSupport()
    return () => { active = false }
  }, [supportId])
